...
void loop() {
  ...
  keyboard.println("run"); // Type "run" followed by a carriage return
  delay(1000); // Wait for the run dialog to come up 
  keyboard.println("www.sparkfun.com"); // Type URL into run window followed by a carriage return.
  delay(10);
  while(1);
